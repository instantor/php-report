...
class Decrypt
{
    ...
    protected static function decrypt($key, $payload, $msg_id)
    {
        ...
        return self::plainDecrypt($payload, $key, $iv, Core::LEGACY_CIPHER_METHOD);
    }
    ...
    public static function receivePostRequest($source, $api_key, &$payload) 
    {
        try 
        {
            ...
            {
                throw new \Exception('Error: Missing POST field(s): ' . implode(', ', $missing_fields));
            }

            if ($fields['source'] !== $source)
            {
                throw new \Exception('Error: Invalid Product ID: ' . $fields['source']);
            }

            if ($fields['encryption'] !== self::ENCRYPTION)
            {
                throw new \Exception('Error: Invalid encryption: ' . $fields['encryption']);
            }
            ...
            if ($calc !== $hash)
            {
                throw new \Exception('Error: Invalid checksum: ' . $hash);
            }
